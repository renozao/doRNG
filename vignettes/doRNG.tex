...
\bibliography{/tmp/Rpkglib_208859b806a6/doRNG/REFERENCES}
...
{\small \Rpkg{doRNG} -- Version 1.6.2}}
...
\end{alltt}
\begin{verbatim}
## [1] 1.569445 2.038106 3.640668 4.329294 5.642481
\end{verbatim}
\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# Loading required package: rngtools\\\#\# Loading required package: methods}}\begin{alltt}
...
\hlstd{s} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=}\hlnum{123}\hlstd{)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
\hlstd{s2} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=}\hlnum{123}\hlstd{)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{## Pass the Normal RNG kind to use within the loop}
\hlcom{# results are identical if not using the Normal kind in the loop}
\hlstd{optsN} \hlkwb{<-} \hlkwd{list}\hlstd{(}\hlnum{123}\hlstd{,} \hlkwc{normal.kind}\hlstd{=}\hlstr{"Ahrens"}\hlstd{)}
\hlstd{resN.U} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=optsN)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\hlstd{resN} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=}\hlnum{123}\hlstd{)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{rnorm}\hlstd{(}\hlnum{3}\hlstd{) \}}
\hlstd{resN1} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=optsN)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{rnorm}\hlstd{(}\hlnum{3}\hlstd{) \}}
\hlstd{resN2} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=optsN)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{rnorm}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\hlstd{s} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{6}\hlstd{)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\hlstd{s} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=seed)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\hlstd{s} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=res)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\end{verbatim}
\begin{alltt}
\hlcom{## use completely custom sequence of seeds (e.g. using RNG "Marsaglia-Multicarry")}
\hlcom{# as a matrix}
...
\end{verbatim}
\begin{alltt}
\hlstd{sM} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=seedM)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\hlstd{sL} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=seedL)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\hlstd{def} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=}\hlnum{123}\hlstd{)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}

\hlcom{# Marsaglia-Multicarry}
\hlkwd{RNGkind}\hlstd{(}\hlstr{'Marsaglia'}\hlstd{)}
\hlstd{mars} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{,} \hlkwc{.options.RNG}\hlstd{=}\hlnum{123}\hlstd{)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\hlcom{# NB: if re-running this vignette you should edit to force using 3 here }
\hlstd{cl} \hlkwb{<-} \hlkwd{makeCluster}\hlstd{(} \hlkwa{if}\hlstd{(}\hlkwd{isManualVignette}\hlstd{())} \hlnum{3} \hlkwa{else} \hlnum{2}\hlstd{)}
\hlkwd{length}\hlstd{(cl)}
\end{alltt}
\begin{verbatim}
## [1] 3
\end{verbatim}
\begin{alltt}
\hlcom{# register new cluster}
\hlkwd{registerDoParallel}\hlstd{(cl)}
\hlkwd{set.seed}\hlstd{(}\hlnum{123}\hlstd{)}
...
## res1     8917     8917     8917     8917     8917
##          8934     8943     8934     8943     8943
## res2     8962     8985     8996     8962     8985
\end{verbatim}
\begin{alltt}
...
\hlstd{res_dopar} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{5}\hlstd{)} \hlopt{%dopar%} \hlstd{\{} \hlkwd{runif}\hlstd{(}\hlnum{3}\hlstd{) \}}
...
\begin{alltt}
\hlcom{# nested loop}
\hlkwd{try}\hlstd{(} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{10}\hlstd{)} \hlopt{%:%} \hlkwd{foreach}\hlstd{(}\hlkwc{j}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{i)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{rnorm}\hlstd{(}\hlnum{1}\hlstd{) \} )}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in foreach(i = 1:10) \%:\% foreach(j = 1:i) \%dorng\% \{: nested/conditional foreach loops are not supported yet.\\\#\# See the package's vignette for a work around.}}\begin{alltt}
\hlcom{# conditional loop}
\hlkwd{try}\hlstd{(} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlnum{10}\hlstd{)} \hlopt{%:%} \hlkwd{when}\hlstd{(i} \hlopt{%%} \hlnum{2} \hlopt{==} \hlnum{0}\hlstd{)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{rnorm}\hlstd{(}\hlnum{1}\hlstd{) \} )}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in foreach(i = 1:10) \%:\% when(i\%\%2 == 0) \%dorng\% \{: nested/conditional foreach loops are not supported yet.\\\#\# See the package's vignette for a work around.}}\end{kframe}
\end{knitrout}
...
\hlstd{n} \hlkwb{<-} \hlnum{10}\hlstd{; p} \hlkwb{<-} \hlnum{5}
...
\hlstd{res} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{n)} \hlopt{%:%} \hlkwd{foreach}\hlstd{(}\hlkwc{j}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{p,} \hlkwc{r}\hlstd{=rng[(i}\hlopt{-}\hlnum{1}\hlstd{)}\hlopt{*}\hlstd{p} \hlopt{+} \hlnum{1}\hlopt{:}\hlstd{p])} \hlopt{%dopar%} \hlstd{\{}
        ...
        \hlcom{# set RNG seed}
    \hlstd{rngtools}\hlopt{::}\hlkwd{setRNG}\hlstd{(r)}
...
\hlstd{res2} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{n)} \hlopt{%:%} \hlkwd{foreach}\hlstd{(}\hlkwc{j}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{p)} \hlopt{%do%}\hlstd{\{}
    \hlcom{# set seed}
        \hlstd{rngtools}\hlopt{::}\hlkwd{setRNG}\hlstd{(rng[[k]])}
...
\hlcom{# run standard nested foreach loop}
\hlstd{res} \hlkwb{<-} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{n)} \hlopt{%:%} \hlkwd{when}\hlstd{(i} \hlopt{%%} \hlnum{2} \hlopt{==} \hlnum{0}\hlstd{)} \hlopt{%:%} \hlkwd{foreach}\hlstd{(}\hlkwc{j}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{i,} \hlkwc{r}\hlstd{=rng[(i}\hlopt{-}\hlnum{1}\hlstd{)}\hlopt{*}\hlstd{i}\hlopt{/}\hlnum{2} \hlopt{+} \hlnum{1}\hlopt{:}\hlstd{i])} \hlopt{%dopar%}\hlstd{\{}
...
\hlkwd{registerDoSEQ}\hlstd{()}
\hlstd{rPar} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{n}\hlstd{,} \hlkwc{s}\hlstd{=}\hlnum{0}\hlstd{)\{} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{n)} \hlopt{%dopar%} \hlstd{\{} \hlkwd{Sys.sleep}\hlstd{(s) \} \}}
\hlstd{rRNG} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{n}\hlstd{,} \hlkwc{s}\hlstd{=}\hlnum{0}\hlstd{)\{} \hlkwd{foreach}\hlstd{(}\hlkwc{i}\hlstd{=}\hlnum{1}\hlopt{:}\hlstd{n)} \hlopt{%dorng%} \hlstd{\{} \hlkwd{Sys.sleep}\hlstd{(s) \} \}}

\hlcom{# run benchmark}
\hlstd{cmp} \hlkwb{<-} \hlkwd{benchmark}\hlstd{(}\hlkwd{rPar}\hlstd{(}\hlnum{10}\hlstd{),} \hlkwd{rRNG}\hlstd{(}\hlnum{10}\hlstd{)}
                        \hlstd{,} \hlkwd{rPar}\hlstd{(}\hlnum{25}\hlstd{),} \hlkwd{rRNG}\hlstd{(}\hlnum{25}\hlstd{)}
                        \hlstd{,} \hlkwd{rPar}\hlstd{(}\hlnum{50}\hlstd{),} \hlkwd{rRNG}\hlstd{(}\hlnum{50}\hlstd{)}
                        \hlstd{,} \hlkwd{rPar}\hlstd{(}\hlnum{50}\hlstd{,} \hlnum{.01}\hlstd{),} \hlkwd{rRNG}\hlstd{(}\hlnum{50}\hlstd{,} \hlnum{.01}\hlstd{)}
            \hlstd{,} \hlkwd{rPar}\hlstd{(}\hlnum{10}\hlstd{,} \hlnum{.05}\hlstd{),} \hlkwd{rRNG}\hlstd{(}\hlnum{10}\hlstd{,} \hlnum{.05}\hlstd{)}
                        \hlstd{,} \hlkwc{replications}\hlstd{=}\hlnum{5}\hlstd{)}
\hlcom{# order by increasing elapsed time}
\hlstd{cmp[}\hlkwd{order}\hlstd{(cmp}\hlopt{$}\hlstd{elapsed), ]}
\end{alltt}
\begin{verbatim}
##              test replications elapsed relative user.self sys.self user.child sys.child
## 1        rPar(10)            5   0.031    1.000     0.031    0.000          0         0
## 3        rPar(25)            5   0.054    1.742     0.055    0.000          0         0
## 2        rRNG(10)            5   0.058    1.871     0.053    0.004          0         0
## 4        rRNG(25)            5   0.093    3.000     0.093    0.000          0         0
## 5        rPar(50)            5   0.096    3.097     0.096    0.000          0         0
## 6        rRNG(50)            5   0.158    5.097     0.153    0.004          0         0
## 9  rPar(10, 0.05)            5   2.597   83.774     0.094    0.000          0         0
## 10 rRNG(10, 0.05)            5   2.655   85.645     0.147    0.003          0         0
## 7  rPar(50, 0.01)            5   2.956   95.355     0.433    0.000          0         0
## 8  rRNG(50, 0.01)            5   3.026   97.613     0.502    0.005          0         0
\end{verbatim}
\end{kframe}
\end{knitrout}
